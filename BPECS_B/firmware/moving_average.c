...
/* Function to calculate moving sum with INT16 */

void init_moving_sum_data_int16(moving_sum_int16 *data, int16_t value) {
    for (uint8_t i = 0; i < NUMBER_OF_SAMPLES; i++) {
        data->data_arr[i] = value;
    }
    data->data_state = INVALID;
...
void add_moving_sum_data_int16(moving_sum_int16 *data, int16_t value) {
    data->data_arr[data->index] = value;
    data->index += 1;
    if (data->index >= NUMBER_OF_SAMPLES) {
        data->index = 0;
...
int32_t get_moving_sum_result_int16(moving_sum_int16 *data) {
    int32_t temp = 0;
    for (uint8_t i=0; i < NUMBER_OF_SAMPLES; i++) {
        temp += data->data_arr[i];
    }
